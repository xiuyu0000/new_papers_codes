...
from src.beam_search import TileBeam
from src.weight_init import normal_weight
from src.weight_init import weight_variable
...
def convert_np_to_tensor_encoder(seq_length):
    """convert numpy tensor to encoder"""
...
class TransformerModel(nn.Cell):
    ...
    def __init__(self,
        ...
        self.first_dense = nn.Dense(in_channels=self.input_feature_size, out_channels=self.embedding_size)
        self.first_layer_preprocess = LayerPreprocess(in_channels=self.embedding_size)
    ...
    def construct(self, source_features, source_mask, target_ids=None, target_mask=None):
        """Transformer with encoder and decoder."""
        enc_seq_length = self.shape(source_features)[1]

        # process source sentence
        source_features = self.first_dense(source_features)
        source_features = self.first_layer_preprocess(source_features)
        src_embedding_output = self.tfm_embedding_postprocessor_for_encoder(source_features)
        ...
        if self.is_training:
            dec_seq_length = self.shape(target_ids)[1]
            future_mask = convert_np_to_tensor_encoder(dec_seq_length)
            ...
            # transformer decoder
            enc_attention_mask = enc_attention_mask[::, 0:dec_seq_length:1, ::]
            decoder_output = self.tfm_decoder(self.cast_compute_type(tgt_embedding_output),
                                              ...
                                              dec_seq_length, enc_seq_length)
            # calculate logits and log_probs
            log_probs = self.projection(decoder_output, embedding_tables, dec_seq_length)
            ret = log_probs
        else:
