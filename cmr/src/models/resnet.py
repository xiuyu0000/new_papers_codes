...
class ResNet(nn.Cell):
    """
    A ResNet-50 model without final fully connected layer
    """
    def __init__(self, block, layers):
        ...
        for m in self.cells():
            if isinstance(m, nn.Conv2d):
                m.weight = Parameter(initializer.initializer(
                    init=initializer.HeNormal(mode='fan_out', nonlinearity='relu'),
                    shape=m.weight.shape, dtype=mindspore.float32), name=m.weight.name)
