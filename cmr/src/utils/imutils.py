...
def crop(img, center, scale, res, rot=0):
    ...
    if rot != 0:
        # Remove padding
        new_img = trans.rotate(new_img, rot, preserve_range=True)
        new_img = new_img[pad:-pad, pad:-pad]

    new_img = trans.resize(new_img, res, preserve_range=True)
    return new_img
...
def uncrop(img, center, scale, orig_shape, rot=0, is_rgb=True):
    ...
    img = Image.fromarray(np.uint8(img)).resize(crop_shape, resample=Image.NEARST)
    img = np.array(img)
