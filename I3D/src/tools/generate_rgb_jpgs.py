...
def video_process(video_file_path, dst_root_path, extract, fps=-1, size=240):
    if extract != video_file_path.suffix:
        return
    ...
    temp = ''.join(i+' 'for i in ffprobe_cmd)
    ffprobe_cmd = temp

    p = subprocess.run(ffprobe_cmd, stdout=subprocess.PIPE, shell=True)
...
def class_process(class_dir_path, dst_root_path, extract, fps=-1, size=240):
    ...
    for video_file_path in sorted(class_dir_path.iterdir()):
        video_process(video_file_path, dst_class_path, extract, fps, size)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'activitynet':
        ...
        status_list = Parallel(
            n_jobs=args.n_jobs,
            backend='threading')(delayed(video_process)(video_file_path, args.dst_path, ext, args.fps, args.size)
        ...
        status_list = Parallel(
            n_jobs=args.n_jobs,
            backend='threading')(delayed(class_process)(class_dir_path, args.dst_path, ext, args.fps, args.size)
