...
def train():
    ...
    d_opt = nn.Adam(pix2pixHD.trainable_params_D, learning_rate=lr,
                    beta1=config.beta1, beta2=config.beta2, loss_scale=1)
    g_opt = nn.Adam(pix2pixHD.trainable_params_G, learning_rate=lr,
                    beta1=config.beta1, beta2=config.beta2, loss_scale=1)
    ...
    for epoch in range(config.niter + config.niter_decay):
        ...
        if (config.niter_fix_global != 0) and (epoch+1 == config.niter_fix_global):
            ...
            g_opt_new = nn.Adam(pix2pixHD.trainable_params_G, learning_rate=lr[epoch:],
                                beta1=config.beta1, beta2=config.beta2, loss_scale=1)
