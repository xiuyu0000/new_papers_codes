...
class ResnetBlock(nn.Cell):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, activation, use_dropout):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3, padding=padding, pad_mode='valid', has_bias=True),
                       norm_layer(dim),
                       activation]
        if use_dropout:
...
class GlobalGenerator(nn.Cell):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        model = [nn.Pad(paddings=paddings, mode=padding_type),
                 nn.Conv2d(input_nc, ngf, kernel_size=7, pad_mode='valid', has_bias=True), norm_layer(ngf), activation]
        # downsample
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.Conv2dTranspose(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=2, has_bias=True),
...
class LocalEnhancer(nn.Cell):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsample_global=3, n_blocks_global=9,
                 n_local_enhancers=1, n_blocks_local=3, norm_layer=nn.BatchNorm2d, padding_type='CONSTANT'):
        ...
        for n in range(1, n_local_enhancers + 1):
            ...
            paddings = ((0, 0), (0, 0), (3, 3), (3, 3))
            model_downsample = [nn.Pad(paddings=paddings, mode=padding_type),
            ...
            if n == n_local_enhancers:
                model_upsample += [nn.Pad(paddings=paddings, mode=padding_type),
                                   nn.Conv2d(ngf, output_nc, kernel_size=7, pad_mode='valid', has_bias=True),
                                   nn.Tanh()]
...
class Encoder(nn.Cell):
    def __init__(self, input_nc, output_nc, ngf=32, n_downsampling=4, norm_layer=nn.BatchNorm2d):
        ...
        model = [nn.Pad(paddings=paddings, mode='CONSTANT'),
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.Conv2dTranspose(ngf * mult, int(ngf * mult / 2),
                                         kernel_size=3, stride=2, has_bias=True),
                      norm_layer(int(ngf * mult / 2)), nn.ReLU()]

        model += [nn.Pad(paddings=paddings, mode='CONSTANT'),
                  nn.Conv2d(ngf, output_nc, kernel_size=7, pad_mode='valid', has_bias=True), nn.Tanh()]
        self.model = nn.SequentialCell(model)
