...
class Discriminator(nn.Cell):
    ...
    Returns:
        Tensor, output tensor.
    """
    def __init__(self, in_planes=3, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
                 use_sigmoid=False, get_inner_feat=False, alpha=0.2,):
        super(Discriminator, self).__init__()
        self.get_inner_feat = get_inner_feat
        self.n_layers = n_layers
        kernel_size = 4
        layers = [[nn.Conv2d(in_planes, ndf, kernel_size, 2, pad_mode='pad', padding=2, has_bias=True),
                   nn.LeakyReLU(alpha)]]
        nf_mult = ndf
        i = 1
        while i < n_layers:
...
class MultiscaleDiscriminator(nn.Cell):
    ...
    Returns:
        Tensor, output tensor.
    """
    def __init__(self, in_planes, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,
                 use_sigmoid=False, num_d=3, get_inner_feat=False):
