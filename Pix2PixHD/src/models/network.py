...
class InstanceNorm2d(nn.Cell):
    ...
    def __init__(self, channel):
        ...
        self.gamma = ms.Parameter(init.initializer(init=ms.Tensor(np.ones(shape=[1, channel, 1, 1], dtype=np.float32)),
                                                   shape=[1, channel, 1, 1]), name='gamma', requires_grad=False)

        self.beta = ms.Parameter(init.initializer(init=init.Zero(), shape=[1, channel, 1, 1]),
                                 name='beta', requires_grad=False)
...
def get_generator(input_nc, output_nc, ngf, netG, n_downsample_global=3, n_blocks_global=9, n_local_enhancers=1,
...
def get_discriminator(input_nc, ndf, n_layer_D, norm='instance',
