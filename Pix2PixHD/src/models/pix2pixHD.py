...
class Pix2PixHD(nn.Cell):
    ...
    def initialize(self):
        ...
        self.netG = get_generator(netG_input_nc, config.output_nc, config.ngf, config.netG,
                                  config.n_downsample_global, config.n_blocks_global,
                                  config.n_local_enhancers, config.n_blocks_local, config.norm)

        # Encoder network
        if self.gen_features:
            self.netE = get_generator(config.output_nc, config.feat_num, config.nef, 'encoder',
                                      config.n_downsample_E, norm=config.norm)
        ...
        if self.is_train:
            ...
            if not config.no_instance:
                netD_input_nc += 1
            self.netD = get_discriminator(netD_input_nc, config.ndf, config.n_layers_D, config.norm,
                                          config.no_lsgan, config.num_D, not config.no_ganFeat_loss)
