...
def create_train_dataset(dataset, batch_size, run_distribute=False, num_parallel_workers=8):
    ...
    if run_distribute:
        ...
        train_ds = de.GeneratorDataset(dataset, column_names=["label", "inst", "image", "feat", "path"],
...
def create_eval_dataset(dataset, batch_size=1):
    ...
    eval_ds = de.GeneratorDataset(dataset, column_names=["label", "inst", "image", "feat", "path"],
                                  shuffle=not config.serial_batches)
