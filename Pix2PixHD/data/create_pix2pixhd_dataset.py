...
for dir_name in os.listdir(left_img_phase_dir):
    for entry in os.scandir(os.path.join((left_img_phase_dir, dir_name))):
        if entry.is_file():
            shutil.copyfile(os.path.join(left_img_phase_dir, dir_name, entry.name),
                            os.path.join(phase_img_dir, entry.name))
    ...
    for entry in os.scandir(os.path.join(gt_fine_phase_dir, dir_name)):
        if entry.is_file():
            ...
            if entry.name.split('_')[4] in 'instanceIds.png':
                shutil.copyfile(os.path.join(gt_fine_phase_dir, dir_name, entry.name),
                                os.path.join(phase_label_dir, entry.name))
            # copy inst
            elif entry.name.split('_')[4] in 'labelIds.png':
                shutil.copyfile(os.path.join(gt_fine_phase_dir, dir_name, entry.name),
                                os.path.join(phase_inst_dir, entry.name))
            else:
                continue
