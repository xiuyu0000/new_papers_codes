...
from mindspore_rl.utils import SoftUpdate, OUNoise
...
class HuberLoss(nn.Cell):
    ...
    def construct(self, predict, label):
        ...
        loss = self.select(cond, 0.5 * self.square(abs_error), self.delta * abs_error - 0.5 * self.square(self.delta))
...
class DDPGActor(Actor):
    ...
    def __init__(self, params=None):
        ...
        self.noise = OUNoise(params['stddev'], params['damping'], self.env.action_space.shape)
...
class DDPGLearner(Learner):
    ...
    def __init__(self, params):
        ...
        # optimizer network
        critic_optimizer = nn.Adam(self.critic_net.trainable_params(), learning_rate=params['critic_lr'])
        ...
        target_params = self.target_actor_net.trainable_params() + self.target_critic_net.trainable_params()
