...
class CUHK03:
    """
    Class for processing CUHK03 dataset
    """
    def __init__(self, root, subset_name):
        """
        :param root: path of CUHK03 dataset
        ...
        self.detected_images_dir = osp.join(self.root, "detected_images")
        if not osp.exists(self.detected_images_dir):
            os.makedirs(self.detected_images_dir)
            self.prepare_detected_images()
        while len(os.listdir(self.detected_images_dir)) != 14097:
            time.sleep(5)
        self.load()

    def _deref(self, mat, ref):
        """_deref"""
        return mat[ref][:].T
        ...
        for campid, camp_ref in enumerate(mat["detected"][0]):
            ...
            for pid in range(num_pids):
                img_paths = self._process_images(
                    mat, camp[pid, :], campid, pid, self.detected_images_dir
                )
                assert img_paths != [], \
