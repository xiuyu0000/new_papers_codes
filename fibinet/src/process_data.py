...
def random_split_trans2h5(in_file_path, output_path, recommendation_dataset_stats,
                          part_rows=2000000, eval_size=0.1, seed=1024):
    ...
    with open(in_file_path, encoding="utf-8") as file_in:
        ...
        for i, line in enumerate(file_in):
            ...
            if i not in eval_indices_set:
                ...
                eval_feature_list.append(ids + wts)
            ...
            if train_label_list and (len(train_label_list) % part_rows == 0):
                ...
                train_label_list = []
                train_part_number += 1
            if eval_label_list and (len(eval_label_list) % part_rows == 0):
                pd.DataFrame(np.asarray(eval_feature_list)).to_hdf(eval_feature_file_name.format(eval_part_number),
                                                                   key="fixed")
                pd.DataFrame(np.asarray(eval_label_list)).to_hdf(eval_label_file_name.format(eval_part_number),
                                                                 key="fixed")
                eval_feature_list = []
                eval_label_list = []
                eval_part_number += 1
...
if __name__ == "__main__":
    ...
    random_split_trans2h5(infile_path, config.output_path, recommendation_dataset_stat,
                          part_rows=2000000, eval_size=config.eval_size, seed=1024)
